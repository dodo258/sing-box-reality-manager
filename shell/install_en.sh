    ...
    if [[ -n "${coreInstallType}" ]]; then
        ...
        if echo ${currentInstallProtocolType} | grep -q 0; then
            if [[ "${coreInstallType}" == 2 ]]; then
                echoMenuHintInline "VLESS+TCP" "TLS"
            else
        ...
        if echo ${currentInstallProtocolType} | grep -q trojan; then
            if [[ "${coreInstallType}" == 1 ]]; then
                echoMenuHintInline "Trojan+TCP" "TLS_Vision"
            fi
        fi
...
switchSSLType() {
    if [[ -z "${sslType}" ]]; then
        echoContent red "\n=============================================================="
        echoMenuHint "1.letsencrypt" "default"
        echoContent yellow "2.zerossl"
        echoMenuHint "3.buypass" "Does not support DNS application"
        echoContent red "================================================== ==============="
...
acmeInstallSSL() {
    if [[ "${dnsSSLStatus}" == "true" ]]; then
        ...
        if [[ -n "${txtValue}" ]]; then
            ...
            echoContent yellow " ---> Please refer to the project README for DNS TXT setup details"
...
v2rayVersionManageMenu() {
    ...
    echoMenuHint "7.Set up automatic update of geo files " "updated every morning"
    echoContent red "================================================== ==============="
...
xrayVersionManageMenu() {
    ...
    echoMenuHint "8.Set up automatic update of geo files " "updated every morning"
    echoContent red "================================================== ==============="
...
EOF
        systemctl daemon-reload
        systemctl enable "${xrayServiceName}.service"
...
handleXray() {
    if [[ "$1" == "restart" ]]; then
        ...
        if isManagedXrayRunning; then
            echoContent green " ---> Xray restarted successfully"
        else
            echoContent red "Xray restart failed"
...
hysteriaPortHopping() {
    ...
    if [[ -z "${hysteriaPortHoppingRange}" ]]; then
        ...
        if [[ -z "${portStart}" || -z "${portEnd}" ]]; then
            ...
            # hysteriaPortHopping
            #else
            iptables -t nat -A PREROUTING -p udp --dport "${portStart}:${portEnd}" -m comment --comment "dodo258_portHopping" -j DNAT --to-destination :${hysteriaPort}

            if iptables-save | grep -q "dodo258_portHopping"; then
...
readHysteriaPortHopping() {
    if [[ -n "${hysteriaPort}" ]]; then
        ...
        if iptables-save | grep -q "dodo258_portHopping"; then
            portHopping=
            portHopping=$(iptables-save | grep "dodo258_portHopping" | cut -d " " -f 8)
...
deleteHysteriaPortHoppingRules() {
    iptables -t nat -L PREROUTING --line-numbers | grep "dodo258_portHopping" | awk '{print $1}' | while read -r line; do
...
customCDNIP() {
    ...
    echoContent yellow "# Notes"
    echoContent yellow "\nTutorial address:"
    echoContent skyBlue "Cloudflare preferred IPs should be tested based on your own network. See the repository README."
...
updateNginxBlog() {
    ...
    echoMenuHint "5.Unlock encrypted music file template " "https://github.com/ix64/unlock-music"
...
updateV2RayCDN() {
    ...
    if [[ -n "${currentAdd}" ]]; then
        ...
        echoContent yellow "2.CNAME who.int"
        echoContent yellow "3.CNAME blog.hostmonit.com"
        echoMenuHint "4.Manual input " "can enter multiple, such as:1.1.1.1,1.1.2.2, cloudflare.com separated by commas"
...
bbrInstall() {
    ...
    echoMenuHint "1.Installation script " "recommended original BBR+FQ"
...
ipv6Routing() {
    ...
    if [[ "${ipv6Status}" == "1" ]]; then
        ...
        echoContent yellow "# Notes\n"
        echoContent yellow "# Notes"
        echoContent yellow "# Notes: please refer to the repository README \n"
...
blacklist() {
    ...
    if [[ "${blacklistStatus}" == "1" ]]; then
        ...
        echoContent red "================================================== ==============="
        echoContent yellow "# Notes\n"
        echoMenuHint "1.Rules support predefined domain name list " "https://github.com/v2fly/domain-list-community"
...
warpRoutingReg() {
    ...
    if [[ "${warpStatus}" == "1" ]]; then
        ...
        echoContent yellow "# Notes\n"
        echoContent yellow "1.All diversion rules set will be deleted"
        echoMenuHintSuffix "2.All outbound rules except WARP " "third party" " will be deleted"
...
routingToolsMenu() {
    ...
    echoContent red "\n================================================ ================="
    echoMenuHint "1.WARP diversion " "Third-party IPv4"
    echoMenuHint "2.WARP diversion " "Third-party IPv6"
...
EOF
        ...
        echoMenuHintSuffix "2.After uninstalling dns unlocking, modify the local " "/etc/resolv.conf" " DNS settings and restart vps\n"
    else
...
customV2RayInstall() {
    ...
    echoMenuHint "1.VLESS+TLS+WS" "CDN"
    echoMenuHint "2.Trojan+TLS+gRPC" "CDN"
    echoMenuHint "3.VMess+TLS+WS" "CDN"
    echoContent yellow "4.Trojan+TLS"
    echoMenuHint "5.VLESS+TLS+gRPC" "CDN"
...
customXrayInstall() {
    echoContent skyBlue "\n========================Personalized installation================== =========="
    echoContent yellow "VLESS is prefixed and 0 is installed by default. If you only need to install 0, just select 0"
    echoMenuHint "0.VLESS+TLS_Vision+TCP" "recommended"
    echoMenuHint "1.VLESS+TLS+WS" "CDN"
    echoMenuHint "2.Trojan+TLS+gRPC" "CDN"
    echoMenuHint "3.VMess+TLS+WS" "CDN"
    echoContent yellow "4.Trojan+TLS"
    echoMenuHint "5.VLESS+TLS+gRPC" "CDN"
    echoMenuHint "7.VLESS+Reality+uTLS+Vision" "recommended"
...
addOtherSubscribe() {
    echoContent yellow "#Notes:"
    echoContent yellow "Please make sure the reverse proxy target is reachable before continuing"
...
switchAlpn() {
    ...
    echoMenuHint "1.When http/1.1 is the first, trojan is available, and some gRPC clients are available " "the client supports manual selection of alpn"
    echoMenuHint "2.When h2 is the first, gRPC is available, and some trojan clients are available " "the client supports manual selection of alpn"
...
initRealityClientServersName() {
    if [[ -n "${domain}" ]]; then
        ...
        echoContent yellow "#Notes"
        echoContent green "Choose common TLS-enabled serverNames and verify them yourself.\n"
...
menu() {
    ...
    echoContent green "Current version: v3.6.4"
    ...
    if echo ${currentInstallProtocolType} | grep -q trojan; then
        echoMenuHint "3.Switch VLESS" "XTLS"
    elif echo ${currentInstallProtocolType} | grep -q 0; then
        echoMenuHint "3.Switch Trojan" "XTLS"
    fi
    ...
    echoMenuHint "7.Account management" "existing nodes: users/subscriptions"
    ...
    echoMenuHint "12.Add new port" "another port on this server"
