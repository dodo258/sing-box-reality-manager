...
# 恢复配置
resetNginxConfig(){
    `cp -Rrf /tmp/dodo258-v2ray-agent/nginx/nginx.conf /etc/nginx/nginx.conf`
    rm -rf /etc/nginx/conf.d/5NX2O9XQKP.conf
    echoColor green "\n恢复配置完毕"
}
# 备份
bakConfig(){
    mkdir -p /tmp/dodo258-v2ray-agent/nginx
    `cp -Rrf /etc/nginx/nginx.conf /tmp/dodo258-v2ray-agent/nginx/nginx.conf`
}
# 安装证书
installTLS(){
    ...
    ~/.acme.sh/acme.sh --installcert -d ${domain} --fullchainpath /tmp/dodo258-v2ray-agent/nginx/${domain}.crt --keypath /tmp/dodo258-v2ray-agent/nginx/${domain}.key --ecc >/dev/null
    if [[ -z `cat /tmp/dodo258-v2ray-agent/nginx/${domain}.key` ]]
    then
        echoColor red "证书key生成失败，请重新运行"
        resetNginxConfig
        exit
    elif [[ -z `cat /tmp/dodo258-v2ray-agent/nginx/${domain}.crt` ]]
    then
    ...
    echoColor green "证书目录/tmp/dodo258-v2ray-agent/nginx"
    ls /tmp/dodo258-v2ray-agent/nginx
...
init(){
    ...
    echoColor green "   12.Github[https://github.com/dodo258/dodo258-v2ray-agent]"
    echoColor red "=============================="
