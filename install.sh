...
checkCentosSELinux() {
    if command -v getenforce >/dev/null 2>&1 && [ "$(getenforce)" == "Enforcing" ]; then
        ...
        echoContent yellow "项目说明：https://github.com/dodo258/sing-box-reality-manager"
        exit 0
    fi
}
checkSystem() {
...
switchSSLType() {
    if [[ -z "${sslType}" ]]; then
        echoContent red "\n=============================================================="
        echoMenuHint "1.letsencrypt" "默认"
        echoContent yellow "2.zerossl"
        echoMenuHint "3.buypass" "不支持DNS申请"
        echoContent red "=============================================================="
...
installTLS() {
    ...
    elif [[ "${websiteTLSAutoMode}" == "true" ]]; then
...
websiteLoadMetadata() {
    if [[ -f "${websiteMetadataFile}" ]]; then
        ...
        . "${websiteMetadataFile}"
        if [[ "${websiteType}" == "blog" && -z "${websiteVariant}" ]]; then
            websiteVariant="blog-basic"
        elif [[ "${websiteType}" == "tools" && -z "${websiteVariant}" ]]; then
            websiteVariant="tools-dev"
        fi
...
websiteGetVariantName() {
    case "${websiteVariant}" in
    blog-basic) echo "简洁技术笔记版" ;;
    blog-docs) echo "文档目录版" ;;
    tools-dev) echo "开发工具版" ;;
    tools-text) echo "文本处理版" ;;
    tools-ops) echo "运维常用版" ;;
    custom-upload) echo "自定义静态站" ;;
    archived-2) echo "归档模板：中文企业站" ;;
    archived-4) echo "归档模板：企业展示站" ;;
    archived-7) echo "归档模板：中文企业站二" ;;
    archived-8) echo "归档模板：个人主页" ;;
    *) echo "默认" ;;
    esac
}

websiteApplyBlogDefaults() {
    case "${websiteVariant}" in
    blog-docs)
        websiteTitle="运维文档与实践手册"
        websiteDescription="围绕 Linux、Docker、Nginx、DNS、Shell 和服务器维护整理的中文技术文档站。"
        ;;
    *)
        websiteVariant="blog-basic"
...
websiteApplyToolDefaults() {
    case "${websiteVariant}" in
    tools-text)
        websiteTitle="常用文本处理小工具"
        websiteDescription="用于整理文本、清理换行、转换大小写和常见编码的中文轻工具站。"
        ;;
    tools-ops)
        websiteTitle="运维常用小工具"
        websiteDescription="面向服务器维护、排障和日常巡检的轻量中文工具与速查页。"
        ;;
    *)
        websiteVariant="tools-dev"
...
websiteSelectBlogVariant() {
    echoContent skyBlue "\n功能 1/2 : 选择博客模板"
    echoContent red "=============================================================="
    echoContent yellow "1.简洁技术笔记版[默认]"
    echoContent yellow "2.文档目录版"
    echoContent red "=============================================================="
    read -r -p "请选择[回车默认1]:" websiteBlogVariantChoice
    case "${websiteBlogVariantChoice}" in
    2) websiteVariant="blog-docs" ;;
    *) websiteVariant="blog-basic" ;;
    esac
    websiteApplyBlogDefaults
}

websiteSelectToolVariant() {
    echoContent skyBlue "\n功能 1/2 : 选择工具站模板"
    echoContent red "=============================================================="
    echoContent yellow "1.开发工具版[默认]"
    echoContent yellow "2.文本处理版"
    echoContent yellow "3.运维常用版"
    echoContent red "=============================================================="
    read -r -p "请选择[回车默认1]:" websiteToolVariantChoice
    case "${websiteToolVariantChoice}" in
    2) websiteVariant="tools-text" ;;
    3) websiteVariant="tools-ops" ;;
    *) websiteVariant="tools-dev" ;;
    esac
    websiteApplyToolDefaults
}
...
websitePromptDomain() {
    local reuseStatus=
    local selectedWebsiteType="${websiteType}"
    local selectedWebsiteVariant="${websiteVariant}"
    ...
    websiteVariant="${selectedWebsiteVariant}"
    websiteTitle="${selectedWebsiteTitle}"
    websiteDescription="${selectedWebsiteDescription}"

    if [[ -n "${existingWebsiteDomain}" ]]; then
        echoContent yellow "\n检测到当前网站域名：${existingWebsiteDomain}"
...
function encodeBase64Input() {
  const input = document.getElementById('b64-input');
  const output = document.getElementById('b64-output');
  if (!input || !output) return;
  output.textContent = input.value ? unicodeToBase64(input.value) : '请输入内容';
}
...
function decodeBase64Input() {
  ...
  if (!input || !output) return;
  try {
    output.textContent = input.value ? base64ToUnicode(input.value.trim()) : '请输入 Base64 内容';
  } catch (error) {
    output.textContent = 'Base64 解码失败：' + error.message;
  }
}
...
function encodeUrlInput() {
  ...
  if (!input || !output) return;
  output.textContent = input.value ? encodeURIComponent(input.value) : '请输入内容';
}
...
function decodeUrlInput() {
  ...
  if (!input || !output) return;
  try {
    output.textContent = input.value ? decodeURIComponent(input.value) : '请输入 URL 编码内容';
  } catch (error) {
    output.textContent = 'URL 解码失败：' + error.message;
  }
}
...
function dedupeLines() {
  ...
  output.textContent = lines.length ? Array.from(new Set(lines)).join('\n') : '请输入需要去重的内容';
}

function cleanTextLines() {
  const input = document.getElementById('clean-input');
...
function convertTextCase(mode) {
  ...
  if (mode === 'upper') {
    ...
    output.textContent = text.split(/\b/).map(chunk => chunk ? chunk.charAt(0).toUpperCase() + chunk.slice(1).toLowerCase() : chunk).join('');
  }
}

async function hashTextSha256() {
  const input = document.getElementById('hash-input');
  const output = document.getElementById('hash-output');
  if (!input || !output) return;
  if (!input.value) {
    output.textContent = '请输入需要计算摘要的内容';
    return;
  }
  const data = new TextEncoder().encode(input.value);
  const digest = await crypto.subtle.digest('SHA-256', data);
  const hashArray = Array.from(new Uint8Array(digest));
  output.textContent = hashArray.map(byte => byte.toString(16).padStart(2, '0')).join('');
}
...
function buildCurlSnippet() {
  ...
  if (!url) {
    output.textContent = '请输入完整的 URL，例如 https://example.com/health';
    return;
  }
  output.textContent = 'curl -I -sS --max-time 15 ' + JSON.stringify(url);
}
EOF
}
...
EOF
    ...
    cat <<'EOF' >"${websitePendingRoot}/2026-05-05-systemd-service-checklist.md"
TITLE: systemd 服务排查清单
DATE: 2026-05-05
SLUG: systemd-service-checklist
SUMMARY: 服务部署后最值得先确认的几项 systemd 状态、日志和自动拉起行为。
---
<p>一台机器上的核心服务越来越多时，systemd 状态检查应该变成固定动作。</p>
<ol>
  <li><code>systemctl status 服务名</code> 看最近状态是否稳定</li>
  <li><code>systemctl is-enabled 服务名</code> 确认是否开机自启</li>
  <li><code>journalctl -u 服务名 -n 100 --no-pager</code> 查看失败原因</li>
  <li>重点看服务文件里是否有 <code>Restart=on-failure</code></li>
</ol>
<p>如果一项服务需要长期跑，最好在上线当天就验证一次重启系统后的自动恢复行为。</p>
EOF

    cat <<'EOF' >"${websitePendingRoot}/2026-05-06-dns-resolution-notes.md"
TITLE: DNS 解析链路排查笔记
DATE: 2026-05-06
SLUG: dns-resolution-notes
SUMMARY: 域名访问异常时，优先拆开看权威解析、递归解析和本地缓存，不要只盯浏览器。
---
<p>DNS 问题经常被误判成网站或服务故障，实际上解析链路本身就可能出错。</p>
<ul>
  <li>先确认权威 DNS 是否已经返回目标记录</li>
  <li>再看本地递归 DNS 是否已同步</li>
  <li>浏览器缓存和系统缓存必要时都要清理</li>
  <li>切换网络环境后再复测一次，能更快排除本地问题</li>
</ul>
<p>如果网站刚迁移 IP，建议记录 TTL 和生效时间，后续排障会轻松很多。</p>
EOF

    cat <<'EOF' >"${websitePendingRoot}/2026-05-07-nginx-access-log-basics.md"
TITLE: Nginx 访问日志基础观察
DATE: 2026-05-07
SLUG: nginx-access-log-basics
SUMMARY: 通过访问日志看真实请求来源、状态码分布和静态资源命中情况。
---
<p>很多问题不需要抓包，先看访问日志就能发现异常模式。</p>
<ul>
  <li><code>tail -f access.log</code> 观察实时请求</li>
  <li>看是否大量集中在某个资源路径返回 <code>404</code></li>
  <li>看 User-Agent 是否正常，还是明显脚本扫描</li>
  <li>看请求来源 IP 是否集中在少数网段</li>
</ul>
<p>站点上线后，访问日志本身也是判断“这个站看起来像不像正常网站”的重要线索。</p>
EOF

    cat <<'EOF' >"${websitePendingRoot}/2026-05-08-server-backup-routine.md"
TITLE: 服务器备份的最低标准
DATE: 2026-05-08
SLUG: server-backup-routine
SUMMARY: 即使是轻量站点，也应该至少备份配置、证书和发布内容。
---
<p>很多人以为静态站点不重要，但真正出问题时，最可惜的往往是细碎配置和更新记录。</p>
<ul>
  <li>备份站点目录和 Nginx 配置</li>
  <li>备份证书文件和续期配置</li>
  <li>备份发布过的文章和待发布文章池</li>
  <li>备份 crontab 和系统服务文件</li>
</ul>
<p>如果条件允许，至少保留一份异地压缩包，并且定期抽查能否恢复。</p>
EOF

    cat <<'EOF' >"${websitePendingRoot}/2026-05-09-ssh-key-rotation-notes.md"
TITLE: SSH 密钥轮换注意事项
DATE: 2026-05-09
SLUG: ssh-key-rotation-notes
SUMMARY: 管理多台机器时，SSH 密钥轮换要先做好别名、授权和回滚方案。
---
<p>SSH 密钥轮换最怕的是直接替换后把自己锁在门外。</p>
<ol>
  <li>先新增新密钥，不要立刻删除旧密钥</li>
  <li>验证新密钥能独立登录后，再清理旧授权</li>
  <li>保留一条密码或控制台回滚路径</li>
  <li>更新本地别名和自动化脚本里引用的 IdentityFile</li>
</ol>
<p>如果一台机器承担多个服务，轮换前最好先确认控制台入口是可用的。</p>
EOF

    cat <<'EOF' >"${websitePendingRoot}/2026-05-10-crontab-maintenance-notes.md"
TITLE: crontab 定时任务维护习惯
DATE: 2026-05-10
SLUG: crontab-maintenance-notes
SUMMARY: 定时任务长期跑起来后，日志位置、失败重试和注释说明都应该固定下来。
---
<p>定时任务不是写完就结束，长期维护时最怕“知道有任务，但不知道它现在在干嘛”。</p>
<ul>
  <li>每条关键任务最好有独立日志文件</li>
  <li>任务行后面加注释标签，方便 grep 和清理</li>
  <li>涉及网络请求的任务要考虑失败后的下次执行影响</li>
  <li>修改 crontab 前先备份旧内容</li>
</ul>
<p>对个人站点来说，定时发布和证书续期这类任务都值得单独留痕。</p>
EOF

    cat <<'EOF' >"${websitePendingRoot}/2026-05-11-certificate-renewal-check.md"
TITLE: 证书续期前后检查点
DATE: 2026-05-11
SLUG: certificate-renewal-check
SUMMARY: HTTPS 证书不是拿到就结束，续期日志、安装路径和 reload 行为都要验证。
---
<p>HTTPS 的稳定性不仅在于申请成功，还在于后续续期能不能持续自动跑通。</p>
<ul>
  <li>确认续期工具的账户配置已经落盘</li>
  <li>确认实际部署到服务使用的证书路径</li>
  <li>确认续期完成后是否自动 reload 对应服务</li>
  <li>确认日志里没有权限或 DNS 验证失败</li>
</ul>
<p>如果一套脚本同时负责服务和证书，最好在部署完成当天就做一次续期链路复查。</p>
EOF

    cat <<'EOF' >"${websitePendingRoot}/2026-05-12-static-site-release-notes.md"
TITLE: 静态站发布流程小结
DATE: 2026-05-12
SLUG: static-site-release-notes
SUMMARY: 静态站的发布流程看起来简单，但每次上线还是应该有固定步骤。
---
<p>静态站没有数据库和应用进程，发布成本低，但也更适合做成标准流程。</p>
<ol>
  <li>先检查站点目录结构是否完整</li>
  <li>再检查 index、about、assets、robots、sitemap</li>
  <li>上线前跑一次本地或服务器端静态检查</li>
  <li>发布后立即看首页、详情页和 HTTPS 状态</li>
</ol>
<p>只要形成固定步骤，静态站的日常维护会比想象中轻松很多。</p>
EOF

    cat <<'EOF' >"${websitePendingRoot}/2026-05-13-shell-script-safety-notes.md"
TITLE: Shell 脚本维护时的安全习惯
DATE: 2026-05-13
SLUG: shell-script-safety-notes
SUMMARY: 线上机器上的脚本修改，最重要的是备份、幂等和明确失败输出。
---
<p>Shell 脚本执行面广，一次小改动就可能影响多个服务，所以修改时要保守。</p>
<ul>
  <li>先备份关键配置文件</li>
  <li>重复执行不应导致结果越来越乱</li>
  <li>失败时要有明确错误提示，不能静默吞掉</li>
  <li>涉及删除动作时要尽量缩小范围</li>
</ul>
<p>能复用现有函数就复用，不要为了一点新功能重写整条稳定链路。</p>
EOF

    cat <<'EOF' >"${websitePendingRoot}/2026-05-14-service-port-planning.md"
TITLE: 服务端口规划建议
DATE: 2026-05-14
SLUG: service-port-planning
SUMMARY: 同一台机器上同时跑网站、代理、订阅和辅助服务时，端口规划最好先写清楚。
---
<p>端口冲突很多时候不是技术问题，而是没有提前规划。</p>
<ul>
  <li>网站优先固定在 <code>80/443</code></li>
  <li>代理节点走随机高位端口</li>
  <li>后台管理和订阅接口单独规划子域名或高位端口</li>
  <li>每次新增服务后，都把监听端口补进文档</li>
</ul>
<p>一台机器职责越多，越要在最开始就把端口规划整理清楚。</p>
EOF

    cat <<'EOF' >"${websitePendingRoot}/2026-05-15-curl-healthcheck-notes.md"
TITLE: curl 探活检查习惯
DATE: 2026-05-15
SLUG: curl-healthcheck-notes
SUMMARY: 网站和服务上线后，用 curl 做一轮最基础的探活检查，往往能提前发现不少问题。
---
<p>很多服务在浏览器里“能打开”，不代表协议、状态码和响应头都对。上线后先用一轮 <code>curl</code> 检查很有必要。</p>
<ul>
  <li>确认首页返回码是否为 200 或预期跳转</li>
  <li>确认 HTTPS 是否能正常协商并返回正文</li>
  <li>确认重定向路径是否符合预期</li>
  <li>确认静态资源不会出现批量 404</li>
</ul>
<p>把最常用的探活命令保留在文档里，比每次临时回忆更稳。</p>
EOF

    cat <<'EOF' >"${websitePendingRoot}/2026-05-16-logrotate-basic-checks.md"
TITLE: logrotate 基础检查点
DATE: 2026-05-16
SLUG: logrotate-basic-checks
SUMMARY: 日志切割看似是小事，但站点和服务长时间运行后，不做切割很容易把磁盘拖满。
---
<p>日志管理往往被忽略，直到磁盘快满时才想起来处理。对于长期运行的网站和服务，<code>logrotate</code> 是很基础的一环。</p>
<ul>
  <li>确认哪些日志文件会持续增长</li>
  <li>确认切割后是否需要 reload 对应服务</li>
  <li>确认保留周期是否符合机器磁盘大小</li>
  <li>确认旧日志是否会被压缩</li>
</ul>
<p>只要提前做好，站点和节点一起跑也不会被日志拖垮。</p>
EOF

    cat <<'EOF' >"${websitePendingRoot}/2026-05-17-http-status-observation.md"
TITLE: HTTP 状态码观察要点
DATE: 2026-05-17
SLUG: http-status-observation
SUMMARY: 站点上线后，最先该看的往往不是页面好不好看，而是状态码和跳转是否自然。
---
<p>很多问题第一眼看不出来，但状态码已经暴露得很明显。首页、详情页、静态资源和 404 页都值得抽样检查。</p>
<ul>
  <li>首页应返回 200 或明确的 301/302 跳转</li>
  <li>静态资源不应频繁返回 404</li>
  <li>不存在页面应返回正常 404，而不是整站空白</li>
  <li>HTTPS 跳转和 www/non-www 策略应保持一致</li>
</ul>
<p>这类检查简单，但非常适合写成固定上线步骤。</p>
EOF

    cat <<'EOF' >"${websitePendingRoot}/2026-05-18-static-assets-checklist.md"
TITLE: 静态资源检查清单
DATE: 2026-05-18
SLUG: static-assets-checklist
SUMMARY: 真正像样的静态站，不只是首页能打开，还要保证资源结构完整。
---
<p>一个正常网站至少要有稳定的资源结构。图标、样式、脚本和说明页越齐全，整体观感越自然。</p>
<ul>
  <li>确认 <code>favicon.ico</code> 存在</li>
  <li>确认 <code>assets</code> 目录里的 CSS 和 JS 正常加载</li>
  <li>确认 <code>robots.txt</code> 和 <code>sitemap.xml</code> 已生成</li>
  <li>确认 about 页能正常访问</li>
</ul>
<p>这类资源不复杂，但缺一个都会让网站显得很临时。</p>
EOF

    cat <<'EOF' >"${websitePendingRoot}/2026-05-19-domain-resolution-cutover.md"
TITLE: 域名切换前的解析确认
DATE: 2026-05-19
SLUG: domain-resolution-cutover
SUMMARY: 新站上线前，域名解析的生效情况最好先确认，再去做证书和回源配置。
---
<p>很多部署流程卡住，不是服务本身有问题，而是域名解析还没稳定落到目标服务器。</p>
<ul>
  <li>先确认权威记录已经改到目标 IP</li>
  <li>再从不同网络环境抽样看解析结果</li>
  <li>确认 TTL 不会拖太久</li>
  <li>证书申请前先确保 80 端口能被正常访问</li>
</ul>
<p>解析确认做在前面，后面的 HTTPS 和站点发布都会顺得多。</p>
EOF

    cat <<'EOF' >"${websitePendingRoot}/2026-05-20-service-restart-notes.md"
TITLE: 服务重启前后该看什么
DATE: 2026-05-20
SLUG: service-restart-notes
SUMMARY: 线上服务改配置后，重启不是结束，关键是重启后立刻做状态和日志确认。
---
<p>很多配置问题不是写文件时暴露，而是在服务重启后才表现出来。所以重启动作后面必须跟检查。</p>
<ul>
  <li>确认 systemd 状态是 active</li>
  <li>确认最近日志没有明显报错</li>
  <li>确认监听端口已经起来</li>
  <li>确认外部探活能通</li>
</ul>
<p>形成固定习惯后，很多“看起来改完了其实没生效”的问题会少很多。</p>
EOF
}
...
websiteGetArticleCategory() {
    local articleFile="$1"
    local articleTitle articleBody
    articleTitle=$(websiteReadArticleField "${articleFile}" "TITLE")
    articleBody=$(websiteReadArticleBody "${articleFile}")
    if echo "${articleTitle} ${articleBody}" | grep -qiE "docker|容器"; then
        echo "Docker"
    elif echo "${articleTitle} ${articleBody}" | grep -qiE "nginx|https|证书"; then
        echo "Nginx/HTTPS"
    elif echo "${articleTitle} ${articleBody}" | grep -qiE "dns|解析"; then
        echo "DNS"
    elif echo "${articleTitle} ${articleBody}" | grep -qiE "shell|bash|ssh"; then
        echo "Shell/SSH"
    elif echo "${articleTitle} ${articleBody}" | grep -qiE "systemd|防火墙|端口|日志|备份|服务器|vps"; then
        echo "运维"
    else
        echo "Linux"
    fi
...
websiteRebuildBlogSite() {
    ...
    while read -r articleFile; do
        ...
        local postTitle postDate postSlug postSummary articleCategory
        postTitle=$(websiteReadArticleField "${articleFile}" "TITLE")
        postDate=$(websiteReadArticleField "${articleFile}" "DATE")
        postSlug=$(websiteReadArticleField "${articleFile}" "SLUG")
        postSummary=$(websiteReadArticleField "${articleFile}" "SUMMARY")
        articleCategory=$(websiteGetArticleCategory "${articleFile}")
        local currentCard
        currentCard=$(cat <<EOF
<article class="post-item">
  ...
  <p>${postSummary}</p>
  <p class="note">分类：${articleCategory}</p>
</article>
EOF
)
        postCards="${postCards}
...
${currentCard}"
        ...
        case "${articleCategory}" in
        "Linux") totalLinux=$((totalLinux + 1)) ;;
        "Docker") totalDocker=$((totalDocker + 1)) ;;
        "Nginx/HTTPS") totalNginx=$((totalNginx + 1)) ;;
        "DNS") totalDNS=$((totalDNS + 1)) ;;
        "Shell/SSH") totalShell=$((totalShell + 1)) ;;
        *) totalOps=$((totalOps + 1)) ;;
        esac
        count=$((count + 1))
    done < <(find "${websitePublishedRoot}" -type f -name "*.md" | sort -r)

    if [[ "${websiteVariant}" == "blog-docs" ]]; then
        categoryCards=$(cat <<EOF
<div class="grid">
  <article class="card"><h2>Linux</h2><p>系统基础、常用命令与日常维护。</p><p class="note">当前内容：${totalLinux} 篇</p></article>
  <article class="card"><h2>Docker</h2><p>容器运行、镜像清理、部署检查。</p><p class="note">当前内容：${totalDocker} 篇</p></article>
  <article class="card"><h2>Nginx / HTTPS</h2><p>站点配置、证书链路和访问行为。</p><p class="note">当前内容：${totalNginx} 篇</p></article>
  <article class="card"><h2>DNS</h2><p>权威解析、递归解析与生效时间排查。</p><p class="note">当前内容：${totalDNS} 篇</p></article>
  <article class="card"><h2>Shell / SSH</h2><p>脚本、密钥轮换和远程连接习惯。</p><p class="note">当前内容：${totalShell} 篇</p></article>
  <article class="card"><h2>运维</h2><p>systemd、日志、防火墙和备份清单。</p><p class="note">当前内容：${totalOps} 篇</p></article>
</div>
EOF
)
    fi
...
<body>
  <div class="site-shell">
    ...
    <section class="hero">
      <span class="badge">中文技术文档站</span>
      <h1>按主题归档的运维知识库</h1>
      <p>${websiteDescription}</p>
    </section>
    <section>
      <h2 class="section-title">目录分类</h2>
      ${categoryCards}
    </section>
    <section>
...
websiteRenderToolPage() {
    local toolPath="$1"
    local toolTitle="$2"
    local toolDescription="$3"
    local toolBadge="$4"
    local toolBody="$5"
    mkdir -p "${websiteSiteRoot}/tools/${toolPath}" >/dev/null 2>&1
    cat <<EOF >"${websiteSiteRoot}/tools/${toolPath}/index.html"
<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>${toolTitle} - ${websiteTitle}</title>
...
<body>
  <div class="site-shell">
    ...
    <section class="hero">
      <span class="badge">${toolBadge}</span>
      <h1>${toolTitle}</h1>
      <p>${toolDescription}</p>
    </section>
    ${toolBody}
...
websiteRenderToolCardsPage() {
    local badgeTitle="$1"
    local heroTitle="$2"
    local heroDescription="$3"
    local cardsHtml="$4"
    cat <<EOF >"${websiteSiteRoot}/index.html"
<!doctype html>
<html lang="zh-CN">
...
<body>
  <div class="site-shell">
    ...
    <section class="hero">
      <span class="badge">${badgeTitle}</span>
      <h1>${heroTitle}</h1>
      <p>${heroDescription}</p>
    </section>
    <section class="tools">
...
websiteRenderToolSiteDev() {
    local cardsHtml
    cardsHtml=$(cat <<'EOF'
<article class="card"><h2><a href="/tools/markdown-summary/">Markdown 内容概览</a></h2><p>快速看一段 Markdown 的行数和首行内容。</p></article>
<article class="card"><h2><a href="/tools/json-formatter/">JSON 格式化</a></h2><p>把压缩 JSON 格式化成清晰结构。</p></article>
<article class="card"><h2><a href="/tools/timestamp-converter/">时间戳转换</a></h2><p>支持秒级和毫秒级时间戳互转。</p></article>
<article class="card"><h2><a href="/tools/base64-codec/">Base64 编解码</a></h2><p>快速编码或解码文本，适合接口联调。</p></article>
<article class="card"><h2><a href="/tools/url-codec/">URL 编解码</a></h2><p>处理查询参数、回调地址和签名内容。</p></article>
<article class="card"><h2><a href="/tools/text-dedup/">文本去重</a></h2><p>按行去重，适合整理域名、IP、规则集。</p></article>
EOF
)
    websiteRenderToolCardsPage "开发工具版" "开发调试常用工具集" "${websiteDescription}" "${cardsHtml}"

    websiteRenderToolPage "markdown-summary" "Markdown 内容概览" "输入一段 Markdown 文本，快速确认内容行数和首行标题。" "Markdown 工具" '<article class="card tool-card"><textarea id="md-input" rows="12" placeholder="# 标题&#10;&#10;这里写正文"></textarea><div class="tool-actions"><button type="button" onclick="summarizeMarkdown()">生成概览</button></div><pre id="md-output">结果会显示在这里</pre></article>'
    websiteRenderToolPage "json-formatter" "JSON 格式化" "适合接口调试时快速查看返回结构。" "JSON 工具" '<article class="card tool-card"><textarea id="json-input" rows="12" placeholder='"'"'{"status":"ok","items":[1,2,3]}'"'"'></textarea><div class="tool-actions"><button type="button" onclick="formatJsonInput()">格式化</button></div><pre id="json-output">结果会显示在这里</pre></article>'
    websiteRenderToolPage "timestamp-converter" "时间戳转换" "支持常见秒级和毫秒级 Unix 时间戳。" "时间工具" '<article class="card tool-card"><input id="ts-input" placeholder="例如 1715000000 或 1715000000000"><div class="tool-actions"><button type="button" onclick="convertTimestamp()">转换</button></div><pre id="ts-output">结果会显示在这里</pre></article>'
    websiteRenderToolPage "base64-codec" "Base64 编解码" "对文本做 Base64 编码与解码，适合处理调试样例。" "编码工具" '<article class="card tool-card"><textarea id="b64-input" rows="10" placeholder="请输入要编码或解码的文本"></textarea><div class="tool-actions"><button type="button" onclick="encodeBase64Input()">编码</button><button type="button" onclick="decodeBase64Input()">解码</button></div><pre id="b64-output">结果会显示在这里</pre></article>'
    websiteRenderToolPage "url-codec" "URL 编解码" "快速处理查询参数、回调地址与签名字段。" "URL 工具" '<article class="card tool-card"><textarea id="url-input" rows="10" placeholder="请输入要编码或解码的内容"></textarea><div class="tool-actions"><button type="button" onclick="encodeUrlInput()">编码</button><button type="button" onclick="decodeUrlInput()">解码</button></div><pre id="url-output">结果会显示在这里</pre></article>'
    websiteRenderToolPage "text-dedup" "文本去重" "按行去重，适合整理规则集、域名、IP 和清单内容。" "文本工具" '<article class="card tool-card"><textarea id="dedupe-input" rows="12" placeholder="每行一条内容"></textarea><div class="tool-actions"><button type="button" onclick="dedupeLines()">按行去重</button></div><pre id="dedupe-output">结果会显示在这里</pre></article>'
}

websiteRenderToolSiteText() {
    local cardsHtml
    cardsHtml=$(cat <<'EOF'
<article class="card"><h2><a href="/tools/text-dedup/">文本去重</a></h2><p>对清单内容按行去重，清理重复项。</p></article>
<article class="card"><h2><a href="/tools/text-cleaner/">换行整理</a></h2><p>清理空行和多余空格，保留干净段落。</p></article>
<article class="card"><h2><a href="/tools/case-converter/">大小写转换</a></h2><p>大写、小写、标题格式快速切换。</p></article>
<article class="card"><h2><a href="/tools/url-codec/">URL 编解码</a></h2><p>处理浏览器地址栏和参数值。</p></article>
EOF
)
    websiteRenderToolCardsPage "文本处理版" "常见文本整理与转换工具" "${websiteDescription}" "${cardsHtml}"

    websiteRenderToolPage "text-dedup" "文本去重" "按行去重，适合整理名单、规则和批量内容。" "文本工具" '<article class="card tool-card"><textarea id="dedupe-input" rows="12" placeholder="每行一条内容"></textarea><div class="tool-actions"><button type="button" onclick="dedupeLines()">按行去重</button></div><pre id="dedupe-output">结果会显示在这里</pre></article>'
    websiteRenderToolPage "text-cleaner" "换行整理" "清理多余空格和空行，让文案结构更干净。" "文本工具" '<article class="card tool-card"><textarea id="clean-input" rows="12" placeholder="把需要整理的文本粘贴到这里"></textarea><div class="tool-actions"><button type="button" onclick="cleanTextLines()">整理文本</button></div><pre id="clean-output">结果会显示在这里</pre></article>'
    websiteRenderToolPage "case-converter" "大小写转换" "适合处理变量名、标题或批量英文内容。" "文本工具" '<article class="card tool-card"><textarea id="case-input" rows="10" placeholder="请输入需要转换的内容"></textarea><div class="tool-actions"><button type="button" onclick="convertTextCase('\''upper'\'')">转大写</button><button type="button" onclick="convertTextCase('\''lower'\'')">转小写</button><button type="button" onclick="convertTextCase('\''title'\'')">标题格式</button></div><pre id="case-output">结果会显示在这里</pre></article>'
    websiteRenderToolPage "url-codec" "URL 编解码" "快速处理中文参数、回调地址和 URL 拼接。" "URL 工具" '<article class="card tool-card"><textarea id="url-input" rows="10" placeholder="请输入要编码或解码的内容"></textarea><div class="tool-actions"><button type="button" onclick="encodeUrlInput()">编码</button><button type="button" onclick="decodeUrlInput()">解码</button></div><pre id="url-output">结果会显示在这里</pre></article>'
}

websiteRenderToolSiteOps() {
    local cardsHtml
    cardsHtml=$(cat <<'EOF'
<article class="card"><h2><a href="/tools/timestamp-converter/">时间戳转换</a></h2><p>巡检日志和告警时间时常用。</p></article>
<article class="card"><h2><a href="/tools/sha256-text/">SHA-256 摘要</a></h2><p>快速校验文本摘要，适合排查配置变化。</p></article>
<article class="card"><h2><a href="/tools/curl-builder/">curl 命令生成</a></h2><p>按 URL 生成常见请求探活命令。</p></article>
<article class="card"><h2><a href="/tools/systemd-cheatsheet/">systemd 速查</a></h2><p>常用状态、日志与自启命令整理。</p></article>
EOF
)
    websiteRenderToolCardsPage "运维常用版" "偏运维排障的轻量工具与速查页" "${websiteDescription}" "${cardsHtml}"

    websiteRenderToolPage "timestamp-converter" "时间戳转换" "支持秒级和毫秒级 Unix 时间戳。" "时间工具" '<article class="card tool-card"><input id="ts-input" placeholder="例如 1715000000 或 1715000000000"><div class="tool-actions"><button type="button" onclick="convertTimestamp()">转换</button></div><pre id="ts-output">结果会显示在这里</pre></article>'
    websiteRenderToolPage "sha256-text" "SHA-256 摘要" "对文本做 SHA-256 摘要，适合比对配置片段或规则内容。" "运维工具" '<article class="card tool-card"><textarea id="hash-input" rows="10" placeholder="请输入需要计算摘要的内容"></textarea><div class="tool-actions"><button type="button" onclick="hashTextSha256()">生成摘要</button></div><pre id="hash-output">结果会显示在这里</pre></article>'
    websiteRenderToolPage "curl-builder" "curl 探活命令生成" "输入 URL 后快速生成一条常用探活命令。" "运维工具" '<article class="card tool-card"><input id="curl-input" placeholder="例如 https://example.com/health"><div class="tool-actions"><button type="button" onclick="buildCurlSnippet()">生成命令</button></div><pre id="curl-output">结果会显示在这里</pre></article>'
    websiteRenderToolPage "systemd-cheatsheet" "systemd 速查" "整理服务状态、自启与日志排查时最常用的一组命令。" "运维速查" '<article class="article"><ul><li><code>systemctl status 服务名</code>：查看当前状态</li><li><code>systemctl is-enabled 服务名</code>：确认开机自启</li><li><code>journalctl -u 服务名 -n 100 --no-pager</code>：查看近端日志</li><li><code>systemctl restart 服务名</code>：重启服务</li><li><code>systemctl daemon-reload</code>：服务文件变更后重载</li></ul><p class="note">这页是轻量速查页，适合站点里承载常用运维命令备忘。</p></article>'
}
...
websiteRenderToolSite() {
    ...
    websiteWriteRobots
    websiteWriteDefaultAboutPage "${websiteTitle}" "${websiteDescription}"
    mkdir -p "${websiteSiteRoot}/tools" >/dev/null 2>&1
    case "${websiteVariant}" in
    tools-text) websiteRenderToolSiteText ;;
    tools-ops) websiteRenderToolSiteOps ;;
    *) websiteVariant="tools-dev"; websiteRenderToolSiteDev ;;
    esac
    websiteRenderSitemap
}
...
websiteDeployBlog() {
    ...
    websiteReloadNginx
    echoContent green "\n ---> 中文技术博客部署成功"
    echoContent yellow " ---> 模板: $(websiteGetVariantName)"
...
websiteShowStatus() {
    ...
    echoContent yellow "模板类型: $(websiteGetVariantName)"
...
xrayVersionManageMenu() {
    ...
    echoMenuHint "8.设置自动更新geo文件" "每天凌晨更新"
...
handleSingBox() {
    if [[ "$1" == "restart" ]]; then
        ...
        if isManagedSingBoxRunning; then
            echoContent green " ---> sing-box重启成功"
        else
            echoContent red "sing-box重启失败"
...
handleXray() {
    if [[ "$1" == "restart" ]]; then
        ...
        if isManagedXrayRunning; then
            ...
            echoContent red "Xray重启失败"
            if [[ -n "${xrayDebugOutput}" ]]; then
                echoContent red "Xray错误输出:"
...
promptManagedMultiRealityPort() {
    echoContent yellow "请输入多实例 Reality 端口[回车随机10000-30000]"
    read -r -p "端口:" managedMultiRealityPort
...
generateManagedMultiRealityUserIdentity() {
    ...
    read -r -p "用户名:" managedMultiRealityUserName
...
deployManagedMultiRealityNode() {
    if [[ -z "${coreInstallType}" ]]; then
        echoContent red " ---> 请先安装并启动一个核心后再部署多实例节点"
        exit 0
    fi

    if [[ "${coreInstallType}" != "1" && "${coreInstallType}" != "2" ]]; then
        echoContent red " ---> 当前核心不支持多实例 Reality 节点"
        exit 0
    fi

    echoContent skyBlue "\n功能 1/1 : 部署多实例 Reality 节点"
    promptManagedMultiRealityPort
    promptManagedMultiRealityTarget
    generateManagedMultiRealityKeypair
    generateManagedMultiRealityUserIdentity
...
showManagedMultiRealityAccounts() {
    ...
    if [[ "${skipSelect}" != "true" ]]; then
        selectManagedMultiRealityNode "查看账号"
    fi
    ...
    jq -c "${usersPath}" "${file}" | while read -r user; do
        ...
        uuid=$(echo "${user}" | jq -r '.id // .uuid')
        echoContent skyBlue "\n ---> 节点:${managedMultiRealitySelectedId}"
        echoContent skyBlue " ---> 账号:${email}"
        echo
        ...
        echoContent green "协议类型:VLESS reality，地址:${address}，publicKey:${publicKey}，shortId:${managedMultiRealityShortID}，serverNames:${targetServer}，端口:${port}，用户ID:${uuid}，传输方式:tcp，账户名:${email}\n"
...
addManagedMultiRealityUser() {
    selectManagedMultiRealityNode "添加用户"
    local file="${managedMultiRealitySelectedFile}"
    local uuid=
    local userName=
    local usersJson=
...
removeManagedMultiRealityUser() {
    selectManagedMultiRealityNode "删除用户"
    local file="${managedMultiRealitySelectedFile}"
    local userCount=
    ...
    read -r -p "请选择要删除的用户编号:" managedMultiRealityDeleteUserIndex
...
deleteManagedMultiRealityNode() {
    ...
    echoContent green " ---> 已删除多实例 Reality 节点: ${managedMultiRealitySelectedId}"
}

managedMultiRealityMenu() {
    if [[ -z "${coreInstallType}" ]]; then
        echoContent red " ---> 请先安装对应核心后再使用多实例 Reality 功能"
        exit 0
    fi

    echoContent skyBlue "\n功能 1/1 : 多实例 Reality 管理"
    echoContent red "\n=============================================================="
    echoContent yellow "# 注意事项"
    echoContent yellow "# 这里只部署独立的 Reality 节点，不是给旧节点补入口端口"
    ...
    echoContent red "=============================================================="
    read -r -p "请选择:" managedMultiRealityAction
...
appendInstalledNodeEntry() {
    ...
    local summary="   核心:${coreName}"
    if [[ -n "${port}" ]]; then
        summary="${summary} 端口:${port}"
    fi
    if [[ -n "${userCount}" && "${userCount}" != "null" ]]; then
        summary="${summary} 用户:${userCount}"
    fi
    if [[ -n "${detail}" ]]; then
...
buildInstalledNodeEntries() {
    ...
    if [[ -f "${singBoxConfigPath}02_VLESS_TCP_inbounds.json" ]]; then
        appendInstalledNodeEntry "2" "0" "VLESS+TLS_Vision+TCP" "$(jq -r '.inbounds[0].listen_port // ""' "${singBoxConfigPath}02_VLESS_TCP_inbounds.json")" "$(jq -r '.inbounds[0].users | length' "${singBoxConfigPath}02_VLESS_TCP_inbounds.json")" "域名:$(jq -r '.inbounds[0].tls.server_name // ""' "${singBoxConfigPath}02_VLESS_TCP_inbounds.json")"
    fi
    if [[ -f "${singBoxConfigPath}03_VLESS_WS_inbounds.json" ]]; then
        appendInstalledNodeEntry "2" "1" "VLESS+TLS+WS" "$(jq -r '.inbounds[0].listen_port // ""' "${singBoxConfigPath}03_VLESS_WS_inbounds.json")" "$(jq -r '.inbounds[0].users | length' "${singBoxConfigPath}03_VLESS_WS_inbounds.json")" "域名:$(jq -r '.inbounds[0].tls.server_name // ""' "${singBoxConfigPath}03_VLESS_WS_inbounds.json") 路径:$(jq -r '.inbounds[0].transport.path // ""' "${singBoxConfigPath}03_VLESS_WS_inbounds.json")"
    fi
    if [[ -f "${singBoxConfigPath}05_VMess_WS_inbounds.json" ]]; then
        appendInstalledNodeEntry "2" "3" "VMess+TLS+WS" "$(jq -r '.inbounds[0].listen_port // ""' "${singBoxConfigPath}05_VMess_WS_inbounds.json")" "$(jq -r '.inbounds[0].users | length' "${singBoxConfigPath}05_VMess_WS_inbounds.json")" "域名:$(jq -r '.inbounds[0].tls.server_name // ""' "${singBoxConfigPath}05_VMess_WS_inbounds.json") 路径:$(jq -r '.inbounds[0].transport.path // ""' "${singBoxConfigPath}05_VMess_WS_inbounds.json")"
    fi
    if [[ -f "${singBoxConfigPath}04_trojan_TCP_inbounds.json" ]]; then
        appendInstalledNodeEntry "2" "4" "Trojan+TLS" "$(jq -r '.inbounds[0].listen_port // ""' "${singBoxConfigPath}04_trojan_TCP_inbounds.json")" "$(jq -r '.inbounds[0].users | length' "${singBoxConfigPath}04_trojan_TCP_inbounds.json")" "域名:$(jq -r '.inbounds[0].tls.server_name // ""' "${singBoxConfigPath}04_trojan_TCP_inbounds.json")"
    fi
    if [[ -f "${singBoxConfigPath}07_VLESS_vision_reality_inbounds.json" ]]; then
        appendInstalledNodeEntry "2" "7" "VLESS+Reality+Vision" "$(jq -r '.inbounds[0].listen_port // ""' "${singBoxConfigPath}07_VLESS_vision_reality_inbounds.json")" "$(jq -r '.inbounds[0].users | length' "${singBoxConfigPath}07_VLESS_vision_reality_inbounds.json")" "目标:$(jq -r '.inbounds[0].tls.reality.handshake.server // ""' "${singBoxConfigPath}07_VLESS_vision_reality_inbounds.json"):$(jq -r '.inbounds[0].tls.reality.handshake.server_port // ""' "${singBoxConfigPath}07_VLESS_vision_reality_inbounds.json")"
    fi
    if [[ -f "${singBoxConfigPath}08_VLESS_vision_gRPC_inbounds.json" ]]; then
        appendInstalledNodeEntry "2" "8" "VLESS+Reality+gRPC" "$(jq -r '.inbounds[0].listen_port // ""' "${singBoxConfigPath}08_VLESS_vision_gRPC_inbounds.json")" "$(jq -r '.inbounds[0].users | length' "${singBoxConfigPath}08_VLESS_vision_gRPC_inbounds.json")" "目标:$(jq -r '.inbounds[0].tls.reality.handshake.server // ""' "${singBoxConfigPath}08_VLESS_vision_gRPC_inbounds.json"):$(jq -r '.inbounds[0].tls.reality.handshake.server_port // ""' "${singBoxConfigPath}08_VLESS_vision_gRPC_inbounds.json")"
    fi
    if [[ -f "${singBoxConfigPath}06_hysteria2_inbounds.json" ]]; then
        appendInstalledNodeEntry "2" "6" "Hysteria2" "$(jq -r '.inbounds[0].listen_port // ""' "${singBoxConfigPath}06_hysteria2_inbounds.json")" "$(jq -r '.inbounds[0].users | length' "${singBoxConfigPath}06_hysteria2_inbounds.json")" "域名:$(jq -r '.inbounds[0].tls.server_name // ""' "${singBoxConfigPath}06_hysteria2_inbounds.json")"
    fi
    if [[ -f "${singBoxConfigPath}09_tuic_inbounds.json" ]]; then
        appendInstalledNodeEntry "2" "9" "Tuic" "$(jq -r '.inbounds[0].listen_port // ""' "${singBoxConfigPath}09_tuic_inbounds.json")" "$(jq -r '.inbounds[0].users | length' "${singBoxConfigPath}09_tuic_inbounds.json")" "域名:$(jq -r '.inbounds[0].tls.server_name // ""' "${singBoxConfigPath}09_tuic_inbounds.json") 拥塞:$(jq -r '.inbounds[0].congestion_control // ""' "${singBoxConfigPath}09_tuic_inbounds.json")"
    fi
    if [[ -f "${singBoxConfigPath}10_naive_inbounds.json" ]]; then
        appendInstalledNodeEntry "2" "10" "Naive" "$(jq -r '.inbounds[0].listen_port // ""' "${singBoxConfigPath}10_naive_inbounds.json")" "$(jq -r '.inbounds[0].users | length' "${singBoxConfigPath}10_naive_inbounds.json")" "域名:$(jq -r '.inbounds[0].tls.server_name // ""' "${singBoxConfigPath}10_naive_inbounds.json")"
    fi
    ...
    if [[ -f "${singBoxConfigPath}13_anytls_inbounds.json" ]]; then
        appendInstalledNodeEntry "2" "13" "AnyTLS" "$(jq -r '.inbounds[0].listen_port // ""' "${singBoxConfigPath}13_anytls_inbounds.json")" "$(jq -r '.inbounds[0].users | length' "${singBoxConfigPath}13_anytls_inbounds.json")" "域名:$(jq -r '.inbounds[0].tls.server_name // ""' "${singBoxConfigPath}13_anytls_inbounds.json")"
    fi
}
...
showInstalledNodes() {
    echoContent skyBlue "\n功能 1/1 : 节点总览"
    echoContent red "\n=============================================================="
    echoContent yellow "# 下面展示的是当前机器实际检测到的已部署节点"
    echoContent yellow "# Xray 的 VLESS+WS / VMess+WS / Trojan+TLS 共享 TLS 入口端口\n"
    echoContent red "=============================================================="
    buildInstalledNodeEntries
    if [[ ${#installedNodeLabels[@]} -eq 0 ]]; then
        echoContent red " ---> 当前没有检测到已部署的节点"
        exit 0
    fi
    echoContent red "=============================================================="
}
...
removeInstalledNodeConfig() {
    ...
    "1:0")
        if [[ -f "${configPath}03_VLESS_WS_inbounds.json" || -f "${configPath}05_VMess_WS_inbounds.json" || -f "${configPath}04_trojan_TCP_inbounds.json" ]]; then
            echoContent red " ---> Xray 的 VLESS+TLS_Vision+TCP 同时承担 1/3/4 的 TLS 入口，不能单独删除"
            echoContent yellow " ---> 请先删除 VLESS+WS / VMess+WS / Trojan+TLS，再删除这个入口节点"
            return 1
        fi
...
deleteInstalledNode() {
    echoContent skyBlue "\n功能 1/1 : 删除指定节点"
    echoContent red "\n=============================================================="
    echoContent yellow "# 删除的是某个协议节点，不是整个脚本"
    echoContent yellow "# 某些基于域名的节点删除后，旧的 Nginx 路径转发残留不会影响其他节点使用\n"
    echoContent red "=============================================================="
    buildInstalledNodeEntries
    if [[ ${#installedNodeLabels[@]} -eq 0 ]]; then
        echoContent red " ---> 当前没有可删除的节点"
        exit 0
    fi
    echoContent red "=============================================================="
    read -r -p "请选择要删除的节点编号:" selectedNodeIndex
    ...
    read -r -p "确认删除【${selectedLabel}】吗？[y/n]:" confirmDeleteNodeStatus
    if [[ "${confirmDeleteNodeStatus}" != "y" ]]; then
        echoContent yellow " ---> 已取消删除"
        exit 0
    fi
...
manageInstalledNodes() {
    echoContent skyBlue "\n功能 1/1 : 节点管理"
    echoContent red "\n=============================================================="
    echoContent yellow "1.查看已部署节点"
    echoContent yellow "2.删除指定节点"
    echoContent yellow "3.切换ALPN"
    echoContent red "=============================================================="
    read -r -p "请选择:" manageInstalledNodeStatus
...
updateNginxBlog() {
    ...
    echoMenuHint "5.解锁加密的音乐文件模版" "https://github.com/ix64/unlock-music"
...
addCorePort() {
    ...
    if [[ "${coreInstallType}" == "2" ]]; then
        echoContent yellow "sing-box 下会先选择要补充新端口的目标协议"
    fi
    ...
    if [[ "${selectNewPortType}" == "1" ]]; then
        if [[ "${coreInstallType}" == "2" ]]; then
            ...
            if [[ ${#singBoxAddedPortFiles[@]} -eq 0 ]]; then
                echoContent yellow " ---> 当前没有已添加的新端口"
                exit 0
            fi
        ...
        if [[ "${coreInstallType}" == "2" ]]; then
            ...
            if [[ ${#singBoxForwardTargetPorts[@]} -eq 0 ]]; then
                ...
                echoContent yellow " ---> 已自动选择目标协议: ${singBoxSelectedTargetLabel}:${singBoxSelectedTargetPort}"
            else
                echoContent yellow "\n请选择要补充新端口的 sing-box 节点"
                local index=0
                ...
                read -r -p "请选择目标协议编号:" singBoxForwardTargetIndex
            ...
            while read -r port; do
                ...
                if [[ "${coreInstallType}" == "2" ]]; then
                    if [[ "${port}" == "${singBoxSelectedTargetPort}" ]]; then
                        echoContent red " ---> 端口 ${port} 与目标节点端口重复，跳过"
                        continue
                    fi
...
EOF
            ...
            if [[ "${coreInstallType}" == "2" ]]; then
                echoContent green " ---> 已为 ${singBoxSelectedTargetLabel} 添加额外端口"
            else
                echoContent green " ---> 添加完毕"
            fi
            reloadCore
        ...
        if [[ "${coreInstallType}" == "2" ]]; then
            mapfile -t singBoxAddedPortFiles < <(listSingBoxAddedPortFiles)
            if [[ ${#singBoxAddedPortFiles[@]} -eq 0 ]]; then
                echoContent yellow " ---> 当前没有可删除的新端口"
                exit 0
            fi
            local index=1
            local file=
...
manageCDN() {
    ...
    if echo "${currentInstallProtocolType}" | grep -qE ",1,|,2,|,3,|,5,|,11,"; then
        ...
        echoContent yellow "# 注意事项"
        echoContent yellow "\n教程地址:"
        echoContent skyBlue "Cloudflare 优选 IP 请根据自身网络环境测试，详情见仓库 README"
        ...
        echoMenuHint "5.手动输入" "可输入多个，比如: 1.1.1.1,1.1.2.2,cloudflare.com 逗号分隔"
...
blacklist() {
    ...
    if [[ "${blacklistStatus}" == "1" ]]; then
        ...
        echoContent red "=============================================================="
        echoContent yellow "# 注意事项\n"
        echoMenuHint "1.规则支持预定义域名列表" "https://github.com/v2fly/domain-list-community"
...
warpRoutingReg() {
    ...
    if [[ "${warpStatus}" == "1" ]]; then
        ...
        echoContent yellow "# 注意事项\n"
        echoContent yellow "1.会删除所有设置的分流规则"
        echoMenuHintSuffix "2.会删除除WARP" "第三方" "之外的所有出站规则\n"
...
customSingBoxInstall() {
    echoContent skyBlue "\n========================个性化安装============================"
    echoContent green "新手提示：无域名优先选 7，有域名直连优先选 0，需要 CDN 再选 1"
    echoMenuHint "0.VLESS+Vision+TCP" "有域名直连推荐"
...
customXrayInstall() {
    echoContent skyBlue "\n========================个性化安装============================"
    echoContent green "新手最推荐：无域名选 7；有域名不走 CDN 选 0；必须套 CDN 再看 1 或 12"
    echoMenuHint "0.VLESS+TLS_Vision+TCP" "有域名直连推荐"
    echoMenuHint "1.VLESS+TLS+WS" "仅CDN推荐"
    #    echoContent yellow "2.Trojan+TLS+gRPC[仅CDN推荐]"
    echoMenuHint "3.VMess+TLS+WS" "仅CDN推荐"
    echoMenuHint "4.Trojan+TLS" "不推荐"
    #    echoContent yellow "5.VLESS+TLS+gRPC[仅CDN推荐]"
    echoMenuHint "7.VLESS+Reality+uTLS+Vision" "无域名/新手最推荐"
    # echoContent yellow "8.VLESS+Reality+gRPC"
    echoMenuHint "12.VLESS+Reality+XHTTP+TLS" "CDN可用/进阶"
    read -r -p "请选择[多选]，[例如:1,2,3]:" selectCustomInstallType
    echoContent skyBlue "--------------------------------------------------------------"
    if echo "${selectCustomInstallType}" | grep -q "，"; then
        echoContent red " ---> 请使用英文逗号分隔"
...
manageAccount() {
    ...
    echoContent red "\n=============================================================="
    echoContent yellow "# 已有节点想继续加用户或查看订阅，请在这里操作，不要重复跑 3.一键无域名Reality"
...
addOtherSubscribe() {
    echoContent yellow "#注意事项:"
    echoContent yellow "请先确认反代目标已正确配置并可访问"
...
initRealityMldsa65() {
    ...
    if ! command -v timeout >/dev/null 2>&1; then
        echoContent yellow " 系统缺少timeout命令，跳过ML-DSA-65探测，避免长时间卡住。"
        return 0
    fi
...
initRealityClientServersName() {
    ...
    if [[ "${coreInstallType}" == "1" || "${selectCoreType}" == "1" ]]; then
        realityDestDomainList="www.japan.travel,www.aniplex.co.jp,www.caltech.edu,www.princeton.edu,www.columbia.edu,www.ucla.edu,www.umich.edu,www.ox.ac.uk,www.nyu.edu,www.asus.com,www.ibm.com,www.ericsson.com,www.tp-link.com,www.synology.com,www.logitech.com,www.seagate.com,www.vmware.com,www.sennheiser.com,www.sothebysrealty.com,www.century21.com"
    ...
    if [[ -n "${realityServerName}" && -z "${lastInstallationConfig}" ]]; then
        ...
        echoContent yellow " ---> 继续使用上次安装的Reality目标域名: ${realityServerName}:${realityDomainPort}"
    fi
...
manageReality() {
    ...
    if ! echo "${currentInstallProtocolType}" | grep -q -E "7,|8," || [[ -z "${coreInstallType}" ]]; then
        echoContent red "\n ---> 请先安装 Reality 协议后再执行此操作，详情请参考仓库 README"
        exit 0
    fi
