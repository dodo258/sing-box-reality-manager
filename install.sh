...
checkCentosSELinux() {
    if command -v getenforce >/dev/null 2>&1 && [ "$(getenforce)" == "Enforcing" ]; then
        ...
        echoContent yellow "项目说明：https://github.com/dodo258/sbox-deploy-tool"
        exit 0
    fi
}
checkSystem() {
...
initDNSAPIConfig() {
    if [[ "$1" == "cloudflare" ]]; then
        echoContent yellow "\n CF_Token 配置请参考仓库 README：https://github.com/dodo258/sbox-deploy-tool\n"
...
nginxBlog() {
    ...
    if [[ -d "${nginxStaticPath}" && -f "${nginxStaticPath}/check" ]]; then
        ...
        if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
            ...
            if [[ "${release}" == "alpine" ]]; then
                wget -q -P "${nginxStaticPath}" "https://raw.githubusercontent.com/dodo258/sbox-deploy-tool/main/fodder/blog/unable/html${randomNum}.zip"
            else
                wget -q "${wgetShowProgressStatus}" -P "${nginxStaticPath}" "https://raw.githubusercontent.com/dodo258/sbox-deploy-tool/main/fodder/blog/unable/html${randomNum}.zip"
            fi
...
updateNginxBlog() {
    ...
    if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
        ...
        if [[ "${release}" == "alpine" ]]; then
            wget -q -P "${nginxStaticPath}" "https://raw.githubusercontent.com/dodo258/sbox-deploy-tool/main/fodder/blog/unable/html${selectInstallNginxBlogType}.zip"
        else
            wget -q "${wgetShowProgressStatus}" -P "${nginxStaticPath}" "https://raw.githubusercontent.com/dodo258/sbox-deploy-tool/main/fodder/blog/unable/html${selectInstallNginxBlogType}.zip"
        fi
            ...
            find /etc/v2ray-agent/subscribe_local/default/* | while read -r email; do
                ...
                if [[ -z "${showStatus}" ]]; then
                    ...
                    if [[ -f "/etc/v2ray-agent/subscribe_local/sing-box/${email}" ]]; then
                        ...
                        if [[ "${release}" == "alpine" ]]; then
                            wget -O "/etc/v2ray-agent/subscribe/sing-box/${emailMd5}" -q "https://raw.githubusercontent.com/dodo258/sbox-deploy-tool/main/documents/sing-box.json"
                        else
                            wget -O "/etc/v2ray-agent/subscribe/sing-box/${emailMd5}" -q "${wgetShowProgressStatus}" "https://raw.githubusercontent.com/dodo258/sbox-deploy-tool/main/documents/sing-box.json"
                        fi
...
initRealityClientServersName() {
    ...
    if [[ "${coreInstallType}" == "1" || "${selectCoreType}" == "1" ]]; then
        realityDestDomainList="gateway.icloud.com,swdist.apple.com,swcdn.apple.com,updates.cdn-apple.com,download-installer.cdn.mozilla.net,addons.mozilla.org,www.mozilla.org,www.booking.com,www.visitcalifornia.com,www.kayak.com,www.japan.travel,www.nintendo.com,store.epicgames.com,www.aniplex.co.jp,www.gundam.info,arxiv.org,www.nature.com,www.berkeley.edu,www.caltech.edu,www.princeton.edu,www.columbia.edu,www.ucla.edu,www.umich.edu,www.ox.ac.uk,www.cam.ac.uk,www.nyu.edu,www.python.org,react.dev,redis.io,www.mongodb.com,www.asus.com,www.ibm.com,www.cloudflare.com,www.atlassian.com,www.jetbrains.com,www.docker.com,www.postman.com,s0.awsstatic.com,d1.awsstatic.com,m.media-amazon.com,www.century21.com,www.sothebysrealty.com,www.zillow.com,www.redfin.com,www.realtor.com,www.ericsson.com,www.netgear.com,www.tp-link.com,www.synology.com,www.qnap.com,www.logitech.com,www.seagate.com,www.vmware.com,www.sennheiser.com"
